...
import { RAFA_CALL_CRM_STATUS_LABELS } from '@/lib/rafacall-crm';
import {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {Object.entries(RAFA_CALL_CRM_STATUS_LABELS).map(([status, label]) => (
...
export default function CrmPage() {
  ...
  const handleDropItem = useCallback(
    async (itemId: string, targetStatus: RafacallCrmStatus) => {
      ...
      setColumns((prev) =>
        moveItemBetweenColumns(prev, itemId, current.status, targetStatus),
      );
      ...
      try {
        ...
        setColumns((prev) =>
          moveItemBetweenColumns(prev, itemId, current.status, targetStatus, updated),
        );
  ...
  const handleModalStatusChange = useCallback(
    async (status: RafacallCrmStatus) => {
      ...
      setModalError('');
      setColumns((prev) =>
        moveItemBetweenColumns(prev, selectedItem.id, selectedItem.crmStatus, status),
      );
