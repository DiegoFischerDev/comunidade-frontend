...
import { COMMUNITY_WHATSAPP_INVITE_GROUPS } from "@/lib/community-whatsapp-groups";
...
export function CommunityWhatsappInviteModal({ open, onClose }: Props) {
  if (!open) return null;
  ...
  return (
    <ModalPortal>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="community-whatsapp-invite-title"
        onMouseDown={(e) => {
      ...
      >
        <div className="w-full max-w-md rounded-2xl bg-card p-5 shadow-xl">
          ...
          <ul className="mt-4 space-y-2">
            {COMMUNITY_WHATSAPP_INVITE_GROUPS.map((group) => (
              <li key={group.id}>
                <a
                  href={group.href}
                  target="_blank"
